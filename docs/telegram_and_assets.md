...
这是 UTC 周一到周六 00:30，也就是北京时间周一到周六 08:30 左右。周二到周六覆盖上一美股交易日，周一用于 A 股开盘前检查周末新闻和中国 ETF 风险。
...
新闻和社交热度只作为解释层。它们会影响报告文字和人工判断线索，暂时不会改变三分类未校准相对倾向。
...
报告中的三类百分比是规则分数经过 softmax 归一化后的未校准相对倾向，合计为 100%，尚未经过历史概率校准，不代表真实发生概率。

后续若要调整评分逻辑，建议先做三件事：

1. 保存足够长的 `history/prediction_snapshots.jsonl` 和 `history/prediction_outcomes.jsonl`。
2. 运行 `python evaluate_predictions.py --history-dir history` 检查分桶统计和相关性。
