...
配置后先运行 `Test Telegram delivery`。成功后再运行 `Weekly memory-stock risk monitor`。
...
ETF 有开盘、最高、最低、收盘和成交量字段，程序会按现有价格评分框架计算。
...
新增基金和 ETF 后，三分类定义、权重和阈值没有改变。基金/ETF 暂时沿用同一套价格趋势评分；新闻情绪仅对支持的美股和美股 ETF 启用。
